...
<template>
  <u-pricing-table :tiers="tiers" :sections="sections">
    <!-- Personalizar el título de un nivel específico -->
    <template #team-title="{ tier }">
      <div class="flex items-center gap-2">
        <u-icon name="i-lucide-crown" class="size-4 text-amber-500" />
        {{ tier.title }}
      </div>
    </template>
    ...
    <template #section-security-title="{ section }">
      <div class="flex items-center gap-2">
        <u-icon name="i-lucide-shield-check" class="size-4 text-green-500" />
  ...
  <u-pricing-plans orientation="vertical" :plans="plans" />
</template>
